...
def save_picture(form_picture, folder='menu_items', size=(800, 600)):
    ...
    # Return only the filename for challenges, else relative path
    if folder == 'challenges' or folder == 'uploads/challenges':
        return picture_filename
    return os.path.join('uploads', folder, picture_filename).replace('\\', '/').replace('\\', '/')
