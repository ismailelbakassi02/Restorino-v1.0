...
class Restaurant(db.Model):
    ...
    menu_images = db.relationship('MenuImage', backref='restaurant', lazy='dynamic', cascade='all, delete-orphan')
...
class UserPostLike(db.Model):
    ...
    # Relationships
    tourist = db.relationship('Tourist', backref='post_likes')
    post = db.relationship('ChallengePost', backref='likes')
    
    # Unique constraint to prevent duplicate likes
    __table_args__ = (db.UniqueConstraint('tourist_id', 'post_id', name='unique_user_post_like'),)
    
    def __repr__(self):
        return f'<UserPostLike {self.tourist.user.name} - Post {self.post_id}>'
...
class AdminLog(db.Model):
    ...
    def __repr__(self):
        return f'<AdminLog {self.action} by {self.admin_user_id} at {self.timestamp}>'
