...
from models import (db, User, Tourist, RestaurantOwner, LocalGuide, Restaurant,
                   ...
                   ChallengeParticipant, ChallengePost, ChallengeComment, UserPostLike, AdminLog, MenuImage)
...
def dashboard():
    ...
    if current_user.user_type == 'tourist':
        ...
        per_page = 5
        reviews_query = Review.query.filter_by(tourist_id=tourist.id).order_by(Review.date.desc())
        reviews_pagination = reviews_query.paginate(page=page, per_page=per_page, error_out=False)
        reviews = reviews_pagination.items
        
        # Get completed challenges and their badges
        ...
        if not owner:
            return redirect(url_for('main.complete_owner_profile'))
        # If admin (super user), show latest 4 admin logs
        if owner.is_super_user:
            admin_logs = AdminLog.query.filter_by(admin_user_id=current_user.id).order_by(AdminLog.timestamp.desc()).limit(4).all()
            return render_template('dashboard.html', owner=owner, admin_logs=admin_logs)
        # Get restaurants owned by this user
        restaurants = Restaurant.query.filter_by(owner_id=owner.id).all()
        return render_template('dashboard.html', owner=owner, restaurants=restaurants)

@main.route('/dashboard/delete-review/<int:review_id>', methods=['POST'])
@login_required
def dashboard_delete_review(review_id):
    """Allow a tourist to delete their own review from the dashboard."""
    review = Review.query.get_or_404(review_id)
    if current_user.user_type != 'tourist':
        abort(403)
    tourist = Tourist.query.filter_by(user_id=current_user.id).first()
    if not tourist or review.tourist_id != tourist.id:
        abort(403)
    
    # Store restaurant ID before deleting the review
...
def restaurant_detail(id):
    ...
    if (not lat or not lng) and restaurant.google_maps_link:
        gml = restaurant.google_maps_link
        # Try to extract 3d<lat>!4d<lng> (actual marker) - like JavaScript version
        m = re.search(r'3d([\d.\-]+)!4d([\d.\-]+)', gml)
        if m:
            lat, lng = m.group(1), m.group(2)
        else:
            # Try to extract @lat,lng from Google Maps link (map center)
            m = re.search(r'@([\d.\-]+),([\d.\-]+)', gml)
            if m:
                lat, lng = m.group(1), m.group(2)
            else:
                # Try to extract q=lat,lng
                m = re.search(r'[?&]q=([\d.\-]+),([\d.\-]+)', gml)
                if m:
                    lat, lng = m.group(1), m.group(2)
    ...
    return render_template('restaurant_detail.html',
                          ...
                          reviews=reviews,
                          photos=photos,
                          menu_images=restaurant.menu_images.all(),
...
def add_menu_item(id):
    ...
    if form.validate_on_submit():
        ...
        if form.photo.data:
            ...
            # Also add as MenuImage for the menu book
            menu_image = MenuImage(restaurant_id=id, image_url=photo_filename)
...
def delete_menu_item(id):
    ...
    return redirect(url_for('main.restaurant_menu', id=restaurant.id))
...
def profile_settings():
    ...
    # Initialize form without populating the FileField
    form = ProfileSettingsForm()
    
    # Populate form fields manually (excluding FileField)
    if request.method == 'GET':
...
def search():
    ...
    page = request.args.get('page', 1, type=int)
    per_page = 4  # Show only 4 restaurants per page
...
def add_location():
    ...
    log = AdminLog(
        ...
        details=f'Location ID: {location.id}, Name: {location.name}'
    )
    db.session.add(log)
    db.session.commit()
...
def verify_guide(id):
    ...
    log = AdminLog(
        admin_user_id=current_user.id,
        action='Verified Guide',
        details=f'Guide ID: {guide.id}, Name: {guide.user.name}'
    )
    db.session.add(log)
    db.session.commit()
...
def like_challenge_post(post_id):
    """Like or unlike a challenge post"""
    if current_user.user_type != 'tourist':
        return jsonify({'success': False, 'message': 'Only tourists can like posts'})
    
    tourist = Tourist.query.filter_by(user_id=current_user.id).first()
    if not tourist:
        return jsonify({'success': False, 'message': 'Tourist profile not found'})
    
    post = ChallengePost.query.get_or_404(post_id)
    
    # Check if already liked
    existing_like = UserPostLike.query.filter_by(
        tourist_id=tourist.id,
        post_id=post_id
    ).first()
...
def food_challenges():
    ...
    # Restrict access for restaurant owners (except admins)
    if current_user.is_authenticated and current_user.user_type == 'owner':
        owner = RestaurantOwner.query.filter_by(user_id=current_user.id).first()
        if not (owner and owner.is_super_user):
            flash('Food challenges are only available for tourists.', 'warning')
            return redirect(url_for('main.index'))
    
    # Get pagination parameters
    page = request.args.get('page', 1, type=int)
    per_page = 3  # Show only 3 posts per page
    ...
    # Get challenge posts for the social wall with pagination
    challenge_posts_query = ChallengePost.query\
    ...
    pagination = challenge_posts_query.paginate(
        page=page, 
        per_page=per_page, 
        error_out=False
    )
    challenge_posts = pagination.items
    ...
    # Get liked posts - SIMPLE
    liked_post_ids = []
    if current_user.is_authenticated and current_user.user_type == 'tourist':
        tourist = Tourist.query.filter_by(user_id=current_user.id).first()
        if tourist:
            likes = UserPostLike.query.filter_by(tourist_id=tourist.id).all()
...
def create_food_challenge():
    ...
    if form.validate_on_submit():
        ...
        image_filename = save_picture(form.challenge_image.data, 'challenges')
...
def delete_review(id):
    ...
    log = AdminLog(
        ...
        details=f'Review ID: {id}, Restaurant ID: {restaurant_id}'
    )
    db.session.add(log)
    db.session.commit()
...
@main.route('/admin/verify-owner/<int:id>', methods=['POST'])
@login_required
def verify_owner(id):
    # Only allow super users/admins
    owner = RestaurantOwner.query.get_or_404(id)
    ...
    log = AdminLog(
        ...
        details=f'Owner ID: {owner.id}, Name: {owner.user.name}'
    )
    db.session.add(log)
    db.session.commit()
    flash('Owner has been verified.', 'success')
    return redirect(url_for('main.admin_dashboard'))
...
@main.route('/delete/review/<int:id>', methods=['POST'])
@login_required
def delete_own_review(id):
    """Allow review owners (tourists) to delete their own reviews"""
    review = Review.query.get_or_404(id)
    # Only the review owner (tourist) can delete
    if current_user.user_type != 'tourist':
        abort(403)
    tourist = Tourist.query.filter_by(user_id=current_user.id).first()
    if not tourist or review.tourist_id != tourist.id:
...
@main.route('/restaurant/<int:id>/set-menu', methods=['POST'])
@login_required
def set_menu(id):
    restaurant = Restaurant.query.get_or_404(id)
    owner = RestaurantOwner.query.filter_by(user_id=current_user.id).first()
    if not owner or restaurant.owner_id != owner.id:
        flash('You can only set the menu for your own restaurants.', 'danger')
        return redirect(url_for('main.restaurant_detail', id=id))

    files = request.files.getlist('menu_images[]')
    if not files or all(f.filename == '' for f in files):
        flash('Please upload at least one menu image.', 'danger')
        return redirect(url_for('main.add_menu_item', id=id))

    for file in files:
        if file and file.filename:
            filename = save_picture(file, folder='menu_items')
            menu_image = MenuImage(restaurant_id=id, image_url=filename)
            db.session.add(menu_image)
    db.session.commit()
    flash('Menu images uploaded successfully!', 'success')
    return redirect(url_for('main.restaurant_detail', id=id))

@main.route('/delete/menu-image/<int:image_id>', methods=['POST'])
@login_required
def delete_menu_image(image_id):
    menu_image = MenuImage.query.get_or_404(image_id)
    restaurant = Restaurant.query.get_or_404(menu_image.restaurant_id)
    is_owner = False
    is_admin = False
    if current_user.is_authenticated and current_user.user_type == 'owner':
        owner = RestaurantOwner.query.filter_by(user_id=current_user.id).first()
    ...
    if not (is_admin or is_owner):
        flash('You are not authorized to delete this menu image.', 'danger')
        return redirect(url_for('main.restaurant_detail', id=restaurant.id))
    # Delete the image file from disk
    delete_image_file(menu_image.image_url)
    db.session.delete(menu_image)
    db.session.commit()
    flash('Menu image deleted successfully!', 'success')
    return redirect(url_for('main.restaurant_menu', id=restaurant.id))

@main.route('/delete-challenge-post/<int:post_id>', methods=['POST'])
@login_required
def delete_challenge_post(post_id):
    """Delete a challenge post (only by owner or super user)"""
    post = ChallengePost.query.get_or_404(post_id)
    can_delete = False
    if current_user.user_type == 'tourist':
        tourist = Tourist.query.filter_by(user_id=current_user.id).first()
        if tourist and post.participant.tourist_id == tourist.id:
            can_delete = True
    ...
    db.session.commit()
    flash('Challenge post deleted successfully.', 'success')
    return redirect(url_for('main.food_challenges'))

@main.route('/delete-food-challenge/<int:challenge_id>', methods=['POST'])
@login_required
def delete_food_challenge(challenge_id):
    """Delete a food challenge (admin only)"""
    owner = RestaurantOwner.query.filter_by(user_id=current_user.id).first()
    if not owner or not owner.is_super_user:
        abort(403)
    challenge = FoodChallenge.query.get_or_404(challenge_id)
    db.session.delete(challenge)
    db.session.commit()
    flash('Food challenge deleted successfully!', 'success')
    return redirect(url_for('main.food_challenges'))

@main.route('/restaurant/<int:id>/menu')
def restaurant_menu(id):
    """Display the menu for a restaurant on a dedicated page"""
    restaurant = Restaurant.query.get_or_404(id)
    menu_items = MenuItem.query.filter_by(restaurant_id=id).all()
    menu_images = restaurant.menu_images.all() if hasattr(restaurant, 'menu_images') else []

    # Serialize menu_items for JSON use in template
    ...
    menu_items_json = [menu_item_to_dict(item) for item in menu_items]

    # Serialize menu_images for JSON use in template (include id)
    menu_images_json = [{'id': img.id, 'image_url': img.image_url} for img in menu_images]

    # Determine if current user can delete menu items/images
    can_delete_menu = False
    if current_user.is_authenticated:
        if current_user.user_type == 'owner':
            owner = RestaurantOwner.query.filter_by(user_id=current_user.id).first()
            if owner and (restaurant.owner_id == owner.id or owner.is_super_user):
                can_delete_menu = True
...
@main.route('/restaurant/<int:id>/menu-setup-choice')
@login_required
def menu_setup_choice(id):
    restaurant = Restaurant.query.get_or_404(id)
    owner = RestaurantOwner.query.filter_by(user_id=current_user.id).first()
    if not owner or restaurant.owner_id != owner.id:
        flash('You can only set the menu for your own restaurants.', 'danger')
        return redirect(url_for('main.restaurant_detail', id=id))
    return render_template('menu_setup_choice.html', restaurant=restaurant)

@main.route('/restaurant/<int:id>/manual-menu-entry', methods=['GET', 'POST'])
@login_required
def manual_menu_entry(id):
    restaurant = Restaurant.query.get_or_404(id)
    owner = RestaurantOwner.query.filter_by(user_id=current_user.id).first()
    if not owner or restaurant.owner_id != owner.id:
        flash('You can only add menu items to your own restaurants.', 'danger')
    ...
    if request.method == 'POST':
        ...
        if photo and photo.filename:
            photo_filename = save_picture(photo, folder='menu_items')
            menu_item.image_url = photo_filename
        db.session.add(menu_item)
        db.session.commit()
        flash('Menu item added successfully!', 'success')
        return redirect(url_for('main.manual_menu_entry', id=id))
    return render_template('manual_menu_entry.html', restaurant=restaurant)